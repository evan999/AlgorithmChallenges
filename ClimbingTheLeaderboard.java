...
class Result {
    ...
    public static List<Integer> climbingLeaderboard(List<Integer> ranked, List<Integer> player) {
        ...
        List<Integer> rankedScores = new ArrayList<>(rankedSet);
        List<Integer> playerRanks = new ArrayList<>();

        int rank = rankedScores.size() + 1;
        for (Integer score : player) {
            for (int ranking = rank - 2; ranking >= 0; ranking--) {
                if (score < rankedScores.get(ranking)) {
                    break;
                } else if (score == rankedScores.get(ranking)) {
                    rank--;
                    break;
                } else {
                    rank--;
